import { SelectOptions } from "@/components/models/select";
    ...
    <div className="relative z-0 w-full mb-6 group">
      <select
        ...
        {options.map((option: SelectOptions, key: number) =>
          <option key={key} value={option.value} selected={option.isSelected} disabled={option.isDisabled}>{option.title}</option>
        )}
      </select>
      <label
        htmlFor={name}
